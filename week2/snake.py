...
def main():
    ...
    while game_on:
        ...
        # Growing mechanism (moving mechanism)
        snake.insert(0, list(snake[0]))  # Make a copy of the head
        if snake[0] == food_pos:
            food_spawn = False
            score += 10 
            snake.insert(0, list(snake[0]))  # Add a new head
        ...
        if (snake[0][0] < 0 or snake[0][0] >= WIDTH or
            ...
            game_over()
        for block in snake[3:]:  # Avoid immediate collision with itself
            if snake[0] == block:
